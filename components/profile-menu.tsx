...
import { User, LogIn, HelpCircle, MessageSquare, LayoutGrid, LogOut, ChevronRight, Sparkles, Key } from 'lucide-react';
...
export function ProfileMenu() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (session?.user) {
        setIsLoggedIn(true);
        setUserEmail(session.user.email || 'developer@dropyoursaas.com');
      }
    });
  ...
  return (
    <>
      <div className="relative inline-block text-left" ref={menuRef}>
        ...
        {isOpen && (
          <div className="absolute right-0 mt-2 w-60 rounded-2xl bg-card border border-border/80 shadow-2xl p-1.5 z-50 animate-in fade-in-0 zoom-in-95 duration-150 text-foreground font-sans">
            {isLoggedIn ? (
              /* LOGGED IN MENU */
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-[10px] font-mono text-muted-foreground truncate">
                        {userEmail || 'creator@dropyoursaas.com'}
                      </div>
                    </div>
                  </div>
                </div>
